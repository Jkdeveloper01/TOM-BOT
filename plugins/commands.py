...
async def start(client, message):
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [
                         [
                            ...
                            [InlineKeyboardButton('🚀 Fast Download / Watch Online🖥️', callback_data=f'generate_stream_link:{file_id}') # this button not working
                         ]
                        ]
                    )
                )
    ...
    if not files_:
        ...
        try:
                ...
                reply_markup=InlineKeyboardMarkup(
                    [
                     [
                         InlineKeyboardButton("Uᴘᴅᴀᴛᴇ Cʜᴀɴɴᴇʟ", url=MAIN_CHANNEL),
                         InlineKeyboardButton("Mᴏᴠɪᴇ Gʀᴏᴜᴘ", url=GRP_LNK)],
                        [InlineKeyboardButton('🚀 Fast Download / Watch Online🖥️', callback_data=f'generate_stream_link:{file_id}') # this button not work
                     ]
                   ]
                )
             )
            ...
            title = ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('Linkz') and not x.startswith('Links') and not x.startswith('{') and not x.startswith('@') and not x.startswith('www'), file.file_name.split()))
    ...
    files = files_[0]
    title = ' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('Linkz') and not x.startswith('Links') and not x.startswith('{') and not x.startswith('@') and not x.startswith('www'), files.file_name.split()))
    ...
    if CUSTOM_FILE_CAPTION:
        try:
            ...
            f_caption=f_caption
    if f_caption is None:
        f_caption = f"{' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('Linkz') and not x.startswith('Links') and not x.startswith('{') and not x.startswith('@') and not x.startswith('www'), files.file_name.split()))}"
    if not await check_verification(client, message.from_user.id) and VERIFY == True:
        btn = [[
            InlineKeyboardButton("Verify", url=await get_token(client, message.from_user.id, f"https://t.me/{temp.U_NAME}?start="))
        ]]
...
@Client.on_message(filters.command("pms") & filters.user(5554564210))
