...
async def filter_languages_cb_handler(client: Client, query: CallbackQuery):
    ...
    if 'url_mode' in settings.keys():
        URL_MODE = settings['url_mode']
    else:
        await save_group_settings(message.chat.id, 'url_mode', False)
        URL_MODE = False
    ...
    #             await client.send_message(MOVIE_GROUP_ID, text=extracted_line.group(1))
        ...
        if settings is not None:
            if query.from_user.id in ADMINS:
                ...
                InlineKeyboardButton('Eɴᴀʙʟᴇ' if settings["auto_filter"] else 'Dɪsᴀʙʟᴇ', 
                                         callback_data=f'setgs#auto_filter#{settings["auto_filter"]}#{grp_id}'),
                ],
                [
